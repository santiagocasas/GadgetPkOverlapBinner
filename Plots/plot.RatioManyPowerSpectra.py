...
list_of_files_and_labels = [[(Base+locpks+binfolderList[0]+list_of_sims[sim]+list_particles[0]+str(list_snaps[sna])+'.txt', 
        simustri+' '+pklabels[pkindex]+' '+list_of_sims[1]+'/'+list_of_sims[0]+' '+list_of_reds[sna]) for sna in snaindex] for sim in simindex]
...
for sna in snaindex:
    simu0=datalist[0][sna][0][:,(0,1)]
    simu1=datalist[1][sna][0][:,(0,1)]
    ratioArra=tools.takeRatio(simu0, simu1)
    axes1.semilogx(ratioArra[:,0],ratioArra[:,1], markers[sna], label=datalist[sim][sna][1], color=collist[sna][sim], lw=3, ms=6, markevery=10, alpha=1.0 )
...
axes1.axis(ymax=1.3, xmin=xmini*0.9, xmax=xmaxi*1.5, ymin=0.9)
...
fig.savefig('nonlinear-Pk_Ratio_'+joinsims+'_'+joinreds+'.pdf')
